...
$rootScope.App.ID = "com.appbuilder.location";
$rootScope.App.Name = "Location (Creada con una versión sin registrar de App Builder de DecSoft)";
$rootScope.App.ShortName = "Location";
...
$rootScope.Location.getGeolocation = function() {
  navigator.geolocation.getCurrentPosition($rootScope.Location.onSuccess, $rootScope.Location.onError, { timeout: $rootScope.Location.Timeout, maximumAge: 3000, enableHighAccuracy: true });
};

$rootScope.Location.onSuccess = function(position) {
  $rootScope.Location.Latitude = position.coords.latitude;
  $rootScope.Location.Altitude = position.coords.altitude;
  $rootScope.Location.Longitude = position.coords.longitude;
  $rootScope.Location.Accuracy = position.coords.accuracy;
  $rootScope.Location.AltitudeAccuracy = position.coords.altitudeAccuracy;
  $rootScope.Location.Heading = position.coords.heading;
  $rootScope.Location.Speed = position.coords.speed;
  $rootScope.$apply();



window.App.Debugger.log("Comienzo del evento Success de Location", "info", -1);

window.App.Debugger.log("SetVar \x22[LatitudeInput.Value]\x22 \x22[Location.Latitude]\x22 \x22String\x22", "info", 1);

$rootScope.LatitudeInput.Value = $rootScope.Location.Latitude;

window.App.Debugger.log("SetVar \x22[LongitudeInput.Value]\x22 \x22[Location.Longitude]\x22 \x22String\x22", "info", 2);

$rootScope.LongitudeInput.Value = $rootScope.Location.Longitude;

window.App.Debugger.log("SetVar \x22[MapsIFrame.Url]\x22 \x22https://www.google.com/maps/embed/v1/search?key=AIzaSyDxsY4GzmMbQZHO-ELpVNgBPE2HynZA2Qg&q=[Location.Latitude],[Location.Longitude]\x22 \x22String\x22", "info", 3);

$rootScope.MapsIFrame.Url = "https://www.google.com/maps/embed/v1/search?key=AIzaSyDxsY4GzmMbQZHO-ELpVNgBPE2HynZA2Qg&q="+$rootScope.Location.Latitude+","+$rootScope.Location.Longitude+"";

window.App.Debugger.log("ApplyModel", "info", 4);

$timeout(function() { $rootScope.$apply(); });

window.App.Debugger.log("Final del evento Success de Location", "info", -2);

};

$rootScope.Location.onError = function(error) {
  $rootScope.Location.Error = error.code;



window.App.Debugger.log("Comienzo del evento Error de Location", "info", -1);

window.App.Debugger.log("If \x22[Location.Error]\x22 \x22==\x22 \x221\x22", "info", 1);

if ($rootScope.Location.Error == 1) {

window.App.Debugger.log("Alert \x22Error\x22 \x22Permission denied. If you are triying this app in the integrated browser, please, open it using a browser from the availables.\x22", "info", 2);

$scope.alert("Error", "Permission denied. If you are triying this app in the integrated browser, please, open it using a browser from the availables.");

window.App.Debugger.log("Else", "info", 3);

} else {

window.App.Debugger.log("Alert \x22Error\x22 \x22Error: [Location.Error]\x22", "info", 4);

$scope.alert("Error", "Error: "+$rootScope.Location.Error+"");

window.App.Debugger.log("EndIf", "info", 5);

}

window.App.Debugger.log("Final del evento Error de Location", "info", -2);
...
window.App.Debugger.log("Comienzo del Button1 Click event", "info", -1);

window.App.Debugger.log("Geolocation \x22Location\x22", "info", 1);

$rootScope.Location.getGeolocation();

window.App.Debugger.log("Final del Button1 Click event", "info", -2);
